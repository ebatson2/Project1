...
get_time <- function(num){
  ...
  time <- paste0(as.character(hr), ':', as.character(min), ':00')
...
val <- JWAP_values$`150`
val
time <- substring(val, 1, regexpr('to', val,)-2)
  ...
  if('JWAP' %in% numer_vars){
    ...
    for(i in seq_along(data_tibble$JWAP)){
      ...
      if(data_tibble$JWAP[i]== 0){
        ...
        print(data_tibble$JWAP_time[i])
      }
    }
  }
  
  if('GRPIP'  %in% numer_vars){
    
  }
  
  data_tibble <- data_tibble |>
      mutate(across(all_of(cat_vars), as.factor)) |>
      mutate(across(numer_vars[numer_vars !="JWAP" & numer_vars!="GRPIP"], as.numeric))
...
We have created several functions (`get_tibble`,`format_num_and_cat_vars`,`query_census_api`,`query_with_years`,`summary.census`,`graph.census`) that will manipulate and process data sets that come from the Public Use Microdata Sample (PUMS) Census API. Lets show a simple example of using them. 
